...
function isBillingBlock(inner) {
  ...
  // Match: {"code":"110"} (billing daily count exceeded — string or numeric),
  // {"code":"112"}, {"code":"10605"}, or a pricingUrl field. The optional
  // opening quote + word boundary keeps numeric codes matchable while
  // "code":1101 stays unmatched.
  return /"code"\s*:\s*"?(110|112|10605)\b/.test(flat) || flat.includes("pricingurl");
}
...
async function peekFirstQoderFrame(reader, decoder) {
  ...
  while (true) {
    ...
    if (nl === -1 && !upstreamDone) {
      ...
      if (consumed.length > SSE_PEEK_BUFFER_LIMIT) return { isError: false, isBilling: false, consumed };
...
async function wrapQoderSSE(response, model, log = null) {
  ...
  const processLine = (line, controller) => {
    ...
    if (statusVal !== 200) {
      ...
      if (isBillingBlock(inner) && !isQueueThrottle(inner)) {
        ...
        // Queue throttle (10605) is congestion with a retry hint, not exhausted
        // quota — it falls through to the 429 path below so clients back off
        // and retry instead of giving up on the key.
