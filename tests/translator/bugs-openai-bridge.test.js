...
describe("bug: Claude → OpenAI bridge data loss", () => {
  ...
  // Fixed in claude-to-openai: image in tool_result becomes a follow-up user
  // turn with an image_url data URI instead of being stringified/dropped.
  it("tool_result with image block is not turned into raw JSON / dropped", () => {
    ...
    expect(JSON.stringify(out)).toContain("data:image/png;base64,ZZZ");
  });
