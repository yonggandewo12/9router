...
// Repo root anchored to this file, not process.cwd(): vitest must run from
// tests/ (that config carries the open-sse/@ aliases), which would make a
// cwd-relative "src/..." resolve to tests/src/... and ENOENT every read.
...
describe("AUDIT-003: Proxy URL validation", () => {
  ...
  it("source should contain validateProxyUrl function", () => {
    const source = readSrc("src/lib/network/outboundProxy.js");
...
describe("AUDIT-018: XSS escaping in OAuth callback", () => {
  it("source should contain escapeHtml function", () => {
    const source = readSrc("src/lib/oauth/utils/server.js");
...
describe("AUDIT-004: Atomic lock file for MITM startup", () => {
  it("manager.js should define LOCK_FILE constant", () => {
    const source = readSrc("src/mitm/manager.js");
